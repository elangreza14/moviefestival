...
func NewGenreRepository(
	...
	return &genreRepository{
		db:           dbPool,
		PostgresRepo: NewRepo[domain.Genre](dbPool),
	}
}
...
func (ur *genreRepository) GetMostViewedGenres(ctx context.Context) ([]domain.ViewedGenre, error) {
	sql := `select 
	...
	res := []domain.ViewedGenre{}
	for rows.Next() {
		genre := domain.ViewedGenre{}
