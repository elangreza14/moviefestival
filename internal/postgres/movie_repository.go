...
func NewMovieRepository(
	...
	if dbPool != nil {
		return &movieRepository{
			db:           dbPool,
			PostgresRepo: NewRepo[domain.Movie](dbPool),
	...
	return &movieRepository{
		db:           querier,
		PostgresRepo: NewRepo[domain.Movie](querier),
	}
}

func (ur *movieRepository) CreateMovieTX(ctx context.Context, movie domain.Movie, artists []string, genres []string) error {
	return ur.txRepo.WithTX(ctx, func(tx QueryPgx) error {
		movieRepo := NewMovieRepository(nil, tx)
...
func (ur *movieRepository) UpdateMovieTX(ctx context.Context, movie domain.Movie, artists []string, genres []string) error {
	return ur.txRepo.WithTX(ctx, func(tx QueryPgx) error {
		movieRepo := NewMovieRepository(nil, tx)
...
func (ur *movieRepository) CreateAndScanId(ctx context.Context, movie *domain.Movie) error {
...
func (ur *movieRepository) GetMovieDetail(ctx context.Context, movieID int) (*domain.Movie, error) {
...
func (ur *movieRepository) GetMoviesWithPaginationAndSearch(ctx context.Context, search, searchBy, orderBy, orderDirection string, page, pageSize int) ([]domain.Movie, error) {
	q := `SELECT 
			m.id,
			m.title,
			m.description,
